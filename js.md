...
0.1+0.2 不等于 0.3？  
计算机在把 0.1 转换为二进制的时候出现了无限循环，十进制的 0.1 无法在计算机中以二进制的形式精确表示，是一个无限循环的二进制数，只能舍弃一定的精确度；0.1 在计算机中保存的只能是一个近似于 0.1 的值，所以就出现了误差
...
Promise/A+ 规范：  
Promise/A+规范是Promise的开放标准，规定了then方法的行为、状态机、异步执行和链式调用的实现细节，确保不同Promise实现可以互操作。
...
暂时性死区：  
在块级作用域中，使用 let 或 const 声明的变量在声明前是不能被访问的，访问会报错，这就是暂时性死区。

数组和链表区别：  
数组是连续的内存空间，访问效率高，插入和删除操作效率低。  
链表是非连续的内存空间，访问效率低，插入和删除操作效率高。
